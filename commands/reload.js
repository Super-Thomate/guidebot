exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-vars
  if (!args || args.length < 1) return message.reply({content: "Must provide a command to reload. Derp."});
  ...
  message.reply({content: `The command \`${command.help.name}\` has been reloaded`});
};
